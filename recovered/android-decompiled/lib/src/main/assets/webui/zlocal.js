...
function fetchT(url, ms) {
    ms = ms || 8000;
    if (typeof AbortController === 'undefined') return fetch(url, { credentials: 'omit' });
...
function renderLogin() {
    ...
    setHtml('<div class="zx-login-screen"><form class="zx-login-card" id="login-form" onsubmit="return false">'
        ...
        + '<h1 class="zx-login-h1">Bem-vindo</h1>'
        + '<div class="zx-login-sub">Adicione sua lista pra começar.</div>'
        + loginFieldsHtml(c && c.code, c && c.user)
        + '</form></div>' + loginFormStyles());
    bindLoginForm(false);
    afterRender();
}
/* Mostra o erro do login E garante que ele fica VISÍVEL (em celular a mensagem
   podia ficar cortada abaixo da dobra — rola até ela). */
function loginErrShow(err, msg) {
    if (!err) return;
    err.textContent = msg;
    try { err.scrollIntoView({ block: 'nearest' }); } catch (e) { try { err.scrollIntoView(false); } catch (e2) {} }
}
function doLogin() {
    var prevCreds = loadCreds() || {};   // pra detectar TROCA de lista (baixo)
    var code = ($('login-code') ? $('login-code').value : '').trim();
    var user = ($('login-user') ? $('login-user').value : '').trim();
    var pass = ($('login-pass') ? $('login-pass').value : '');
    var err = $('login-err'), btn = $('login-submit');
    if (btn && btn.className.indexOf('is-loading') >= 0) return;   // já logando (Enter + submit do form podem disparar juntos)
    if (!code || !user || !pass) { loginErrShow(err, t('Preencha tudo.')); return; }
    if (btn) { btn.innerHTML = te('Entrando…'); btn.className += ' is-loading'; }
    S.code = code; S.user = user; S.pass = pass; S.did = getDid();
    api('resolve', '', 12000).then(function (d) {
        if (btn) { btn.innerHTML = te('Entrar'); btn.className = 'zx-login-btn'; }
        if (d && d.error === 'license') { saveCreds(); if (applyPush(d)) return; renderPaywall(d); return; }   // vencido → paywall COMPLETO (QR+MAC); se veio push (lista enviada pro MAC), aplica antes
        if (!d || !d.ok || !d.dns || !d.dns.base) { loginErrShow(err, t('Não foi possível entrar. Confira os dados.')); return; }
        // TROCOU de servidor (outro DNS) ou de usuário → catálogo VEM FRESCO:
        // limpa a RAM (S.cat) + o cache em disco (igual push/Recarregar/migração).
        // Sem isto a home seguia com o catálogo da lista ANTERIOR até fechar o
        // app (RAM de SPA). Mesma lista re-logada → não limpa (segue instantâneo).
        if ((S.server && d.dns.base !== S.server) || (prevCreds.user && prevCreds.user !== user)) {
            S.cat = { movies: null, series: null, live: null };
            try { if (global.HdxCache) HdxCache.bust(); } catch (e) {}
        }
        S.favDirty = { live: [], movie: [], series: [] };   // login limpo: adota favoritos do servidor
        applyResolve(d, false); saveSnap(d); saveCreds(); go('/home', true);
    }).catch(function () { if (btn) { btn.innerHTML = te('Entrar'); btn.className = 'zx-login-btn'; } loginErrShow(err, t('Sem conexão. Tente de novo.')); });
}
/* ---- LISTAS ---- A home abre SEM login; a lista é adicionada/trocada AQUI (1 lista
   só). Tem lista -> CARD; não tem (ou ?edit) -> formulário DENTRO da página. Igual
   Samsung/LG/Roku. Campos do login extraídos pra reuso (renderLogin + renderLists). */
/* Campos de login com INPUTS de verdade (celular abre o teclado no toque). */
function loginFieldsHtml(codeVal, userVal) {
    var cv = codeVal ? attr(codeVal) : '';
    var uv = userVal ? attr(userVal) : '';
    return '<div class="zx-field"><label for="login-code">URL ou código do servidor</label>'
        + '<input type="text" id="login-code" class="zx-in" value="' + cv + '" placeholder="código ou http://servidor:porta" autocomplete="off" autocapitalize="none" spellcheck="false" enterkeyhint="next" autofocus></div>'
        + '<div class="zx-field"><label for="login-user">Usuário</label>'
        + '<input type="text" id="login-user" class="zx-in" value="' + uv + '" placeholder="seu usuário" autocomplete="off" autocapitalize="none" spellcheck="false" enterkeyhint="next"></div>'
        + '<div class="zx-field"><label for="login-pass">Senha</label>'
        + '<input type="password" id="login-pass" class="zx-in" placeholder="sua senha" autocomplete="off" enterkeyhint="go"></div>'
        + '<button type="submit" class="zx-login-btn" id="login-submit">Entrar</button>'
        + '<div id="login-err" class="zx-login-err"></div>';
}
function bindLoginForm(skipPrefill) {
    var codeI = $('login-code'), userI = $('login-user'), passI = $('login-pass'), submitBtn = $('login-submit');
    // Enter/ação do teclado: código→usuário→senha→LOGA. Alguns IMEs do Android só
    // entregam o Enter no keyup (ou nem mandam keydown 13) → escuta os DOIS, com
    // trava anti-duplo. Na senha, fecha o teclado (blur) e loga direto.
    var lastEnter = 0;
    function onEnter(el, next) {
        ...
        function h(e) {
            ...
            var now = Date.now(); if (now - lastEnter < 400) return; lastEnter = now;
            if (next) { try { next.focus(); } catch (_) {} }
            else { try { el.blur(); } catch (_) {} doLogin(); }
        }
        el.addEventListener('keydown', h);
        el.addEventListener('keyup', h);
    }
    onEnter(codeI, userI); onEnter(userI, passI); onEnter(passI, null);
